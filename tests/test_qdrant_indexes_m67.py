...
indexek jonnek letre; uj kollekcion a kollekcio + mind a 13 index; ha minden
...
def test_existing_collection_creates_only_missing_indexes():
    ...
    assert sorted(_index_fields(fake)) == ["available", "cat_tags", "category", "facets", "p_kijelzo", "p_max_meret", "p_szin", "p_tipus", "sku", "type", "url", "usage"]
...
def test_new_collection_gets_all_indexes():
    ...
    assert sorted(_index_fields(fake)) == ["available", "cat_tags", "category", "client_id", "facets", "p_kijelzo", "p_max_meret", "p_szin", "p_tipus", "sku", "type", "url", "usage"]
