...
# --- m26: Unas raktarankenti keszlet-note (_unas_wh_note) — eles XML-mintakbol ---
import xml.etree.ElementTree as _ET

_WHN_PROD_XML = """<Product><Id>1138305179</Id><Sku>731717</Sku><Stocks>
...
def _whn_prod():
    return _ET.fromstring(_WHN_PROD_XML)


def test_whn_aktiv_qty_raktar_bekerul():
    note = lp._unas_wh_note(_whn_prod(), _WHN_MAP, None)
    assert "Központi raktár (2 munkanap): 7 db" in note


def test_whn_inaktiv_stock_szurve():
    note = lp._unas_wh_note(_whn_prod(), _WHN_MAP, None)
    assert "43" not in note


def test_whn_nulla_keszlet_szurve():
    note = lp._unas_wh_note(_whn_prod(), _WHN_MAP, None)
    assert "Beszállítói" not in note


def test_whn_ismeretlen_raktar_szurve():
    assert lp._unas_wh_note(_whn_prod(), {"6160206": ("X", "")}, None) == ""
