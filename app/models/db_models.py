...
class Usage(Base):
    ...
    period: Mapped[str] = mapped_column(String)          # pl. "2026-06" (Europe/Budapest)
    conversations: Mapped[int] = mapped_column(Integer, default=0)
    messages: Mapped[int] = mapped_column(Integer, default=0, server_default="0")
...
class Unanswered(Base):
    ...
    score: Mapped[float | None] = mapped_column(Float)              # retrieval top dense score
    reasons: Mapped[list | None] = mapped_column(JSONB)            # ⊆ {low_score, collect_lead, order_form}
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())


class Message(Base):
    """Beszélgetés-napló — minden /chat üzenet-turn (m22). Retention: 30 nap."""
